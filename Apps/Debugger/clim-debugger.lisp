...
;;; Modified (2016) by Bruce-Robert Fenn Pocock (mostly for æsthetics)
...
;;; Slime project, and the graphical  layout is also heavily inspired by
;;; Slime.  Because   of  Slime  I   hope  that  this  works   on  other
...
;;; Problems/todo:
;;;
;;; - Elliott Johnson is to be thanked for the nice scroll-bars, but for
;;;   some reason  they don't remember  their position when  clicking on
;;;   a stack-frame or "more".
;;;
;;; - The break function  does not use the  clim-debugger --> Christophe
;;;   Rhodes   was   kind   enough   to  inform   me   that   on   SBCL,
;;;   SB-EXT:*INVOKE-DEBUGGER-HOOK* takes care off this problem. I still
;;;   don't know if this is a problem with other compilers.
;;;
;;; - "Eval in frame" is not supported. I don't know of a good way to do
;;; - this currently.
;;;
...
;;; through the global variable *returned-restart*, this is not the best
;;; solution, but I  do not know how  of a better way to  return a value
;;; from a clim frame, when it exits.
;;;
;;; - There need  to be  added keyboard shortcuts.  'q' should  exit the
;;;   debugger with an abort. '0', '1'  and so forth should activate the
;;;   restarts, like Slime. Maybe is should be possible to use the arrow
;;;   keys as well. Then  we have to add a notion  of the current frame.
;;;   Would this be useful?
;;;
;;; - The   backtrace    seems   to   start   at    the   wrong   place?
;;; - compared to Slime?
